...
const KomPoints = ({ pcsRaceId }: Props) => {
  const { roles, points } = usePointData({
    pcsRaceId,
    matrixRole: MatrixRole.KOM_POINTS,
  });

  return (
    <Box>
      <Box textTransform="initial" fontSize="sm" mb={4}>
        <Text translateText="at_the_end_of_the_race_points_are_given_according_to_the_rider_kom" />
