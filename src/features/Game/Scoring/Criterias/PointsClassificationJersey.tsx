...
const PointsClassificationJersey = ({ pcsRaceId }: Props) => {
  const { t } = useTranslation();
  const { matrixRole: data, stageNumber } = usePointData({
    pcsRaceId,
    matrixRole: MatrixRole.POINTS_CLASSIFICATION_JERSEY,
  });
  ...
  return (
    <Box>
      <Box textTransform="initial" fontSize="sm" mb={4}>
        <Text translateText="at_each_stage_points_are_given_to_the_rider_if_he_is_the_leader_of_the_point" />
      ...
      <TableContainer>
        ...
        >
          ...
          <Tbody>
            <Tr>
              <Td>{t('per_day_as_sprint_leader')}</Td>
              {_.map(roles, (role) => (
                <Td key={role.name}>
                  {fixedNumber(
...
export default PointsClassificationJersey;
