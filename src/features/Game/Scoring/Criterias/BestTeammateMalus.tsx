...
const BestTeammateMalus = ({ pcsRaceId }: Props) => {
  const { t } = useTranslation();
  const { matrixRole: data } = usePointData({
    pcsRaceId,
    matrixRole: MatrixRole.BEST_TEAMMATE_MALUS,
  });
  ...
  return (
    <Box>
      <Text
        ...
        fontSize="sm"
        mb={4}
        translateText="at_the_end_of_the_race_malus_is_given"
      />
      <TableContainer>
        <Table
          sx={{
        ...
        >
          ...
          <Tbody>
            <Tr>
              ...
              {_.map(roles, (role) => (
                <Td key={role.name}>
                  {`${role?.value ? '-' : ''}${fixedNumber(
                    (role?.value / 100) * data?.maxPoint,
                    2
                  )}`}
                </Td>
              ))}
...
export default BestTeammateMalus;
